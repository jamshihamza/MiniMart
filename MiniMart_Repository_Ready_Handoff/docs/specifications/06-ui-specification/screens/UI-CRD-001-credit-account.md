...
Limit/terms/exposure/ledger view.

## Authorization

Role labels guide navigation only. Actual action visibility follows API
permission/capability data. Relevant permission codes: `credit.post`,
...
1.  Context/header: Store/Counter/Business Date when relevant, user,
    local/cloud state.
2.  Primary task region: Credit Account.
...
-   `API-CRD-008` POST
    `/api/v1/customer-collections/{collectionId}:post` --- Post
    collection + credit ledger + cash/financial effect
...
-   `API-CRD-006`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, CREDIT_CHANGED, CREDIT_POLICY_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT`.
-   `API-CRD-008`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, CREDIT_CHANGED, CREDIT_POLICY_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT`.
