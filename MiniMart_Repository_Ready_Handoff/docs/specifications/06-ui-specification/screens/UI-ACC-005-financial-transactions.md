# UI-ACC-005 --- Financial Transactions
...
Receipts/payments and movement history.
...
1.  Context/header: Store/Counter/Business Date when relevant, user,
    local/cloud state.
2.  Primary task region: Financial Transactions.
...
-   `API-ACC-030` GET
    `/api/v1/accounting/financial-transactions/{financialTransactionId}`
    --- Get financial transaction and source linkage
