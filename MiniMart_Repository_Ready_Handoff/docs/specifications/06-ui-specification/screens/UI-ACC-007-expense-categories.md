...
1.  Context/header: Store/Counter/Business Date when relevant, user,
    local/cloud state.
2.  Primary task region: Expense Categories.
...
-   `API-ACC-015` GET `/api/v1/accounting/expense-categories` ---
    List/search expense categories
-   `API-ACC-016` POST `/api/v1/accounting/expense-categories` ---
    Create expense category
-   `API-ACC-017` PATCH
    `/api/v1/accounting/expense-categories/{expenseCategoryId}` ---
...
-   `API-ACC-006`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT, PERMISSION_DENIED, RESOURCE_NOT_FOUND`.
-   `API-ACC-016`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT, PERMISSION_DENIED, RESOURCE_NOT_FOUND`.
-   `API-ACC-017`: disable duplicate submit; preserve idempotency
    identity; handle
