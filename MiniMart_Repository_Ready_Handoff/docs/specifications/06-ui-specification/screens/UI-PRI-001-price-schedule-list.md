# UI-PRI-001 --- Price Schedule List

**Surface:** Back Office\
**Area:** Pricing\
**Primary roles/personas:** Manager, Admin\
**Offline behavior:** READ_WRITE_LOCAL

## Purpose

Browse effective-dated store prices.
...
1.  Context/header: Store/Counter/Business Date when relevant, user,
    local/cloud state.
2.  Primary task region: Price Schedule List.
...
-   `API-PRI-007` POST
    `/api/v1/pricing/schedules/{priceScheduleId}:deactivate` ---
    Deactivate schedule where allowed
-   `API-PRI-008` GET `/api/v1/pricing/items/{itemId}/current` --- Get
    current store price snapshot
...
-   `API-PRI-006`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT, PERMISSION_DENIED, RESOURCE_NOT_FOUND`.
-   `API-PRI-007`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT, PERMISSION_DENIED, RESOURCE_NOT_FOUND`.
