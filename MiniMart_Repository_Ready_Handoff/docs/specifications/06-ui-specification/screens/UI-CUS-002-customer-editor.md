...
Maintain company-wide customer master.
...
1.  Context/header: Store/Counter/Business Date when relevant, user,
    local/cloud state.
2.  Primary task region: Customer Editor.
...
-   `API-CUS-001` GET `/api/v1/customers` --- Search/list customers
-   `API-CUS-002` POST `/api/v1/customers` --- Create customer
-   `API-CUS-003` GET `/api/v1/customers/{customerId}` --- Get customer
-   `API-CUS-004` PATCH `/api/v1/customers/{customerId}` --- Update
    customer with conflict-aware version check
-   `API-CUS-005` POST `/api/v1/customers/{customerId}:activate` ---
    Activate customer
-   `API-CUS-006` POST `/api/v1/customers/{customerId}:deactivate` ---
    Deactivate customer without deleting history
...
-   `API-CRD-006`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, CREDIT_CHANGED, CREDIT_POLICY_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT`.
-   `API-CUS-002`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, CREDIT_CHANGED, CREDIT_POLICY_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT`.
-   `API-CUS-004`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, CREDIT_CHANGED, CREDIT_POLICY_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT`.
-   `API-CUS-005`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, CREDIT_CHANGED, CREDIT_POLICY_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT`.
-   `API-CUS-006`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, CREDIT_CHANGED, CREDIT_POLICY_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT`.
