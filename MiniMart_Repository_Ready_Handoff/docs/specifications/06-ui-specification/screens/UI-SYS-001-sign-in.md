...
**Surface:** Shared\
**Area:** System\
**Primary roles/personas:** All users\
**Offline behavior:** LOCAL_AUTH_REQUIRED

## Purpose

Authenticate to the enrolled Store Node.
...
-   `API-IAM-015` POST `/api/v1/iam/overrides` --- Create scoped manager
    override authorization
-   `API-IAM-016` GET `/api/v1/iam/overrides/{overrideId}` --- Get
    override state/evidence
-   `API-IAM-017` POST `/api/v1/iam/support-sessions` --- Create
    time-limited support authorization
-   `API-IAM-018` POST
    `/api/v1/iam/support-sessions/{supportSessionId}:revoke` --- Revoke
...
-   `API-IAM-005`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT, PERMISSION_DENIED, RESOURCE_NOT_FOUND`.
-   `API-IAM-015`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT, PERMISSION_DENIED, RESOURCE_NOT_FOUND`.
-   `API-IAM-017`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT, PERMISSION_DENIED, RESOURCE_NOT_FOUND`.
-   `API-IAM-018`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT, PERMISSION_DENIED, RESOURCE_NOT_FOUND`.
