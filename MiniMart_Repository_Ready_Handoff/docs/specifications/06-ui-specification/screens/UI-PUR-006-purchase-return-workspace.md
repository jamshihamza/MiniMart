# UI-PUR-006 --- Purchase Return Workspace

**Surface:** Back Office\
**Area:** Purchasing\
**Primary roles/personas:** Purchase, Manager\
**Offline behavior:** READ_WRITE_LOCAL

## Purpose

Create source-linked purchase return using current WAC rules.
...
`procurement.post`, `procurement.read`, `procurement.write`

## Layout / information hierarchy

1.  Context/header: Store/Counter/Business Date when relevant, user,
    local/cloud state.
2.  Primary task region: Purchase Return Workspace.
...
-   `API-PUR-011` GET `/api/v1/procurement/purchase-returns` ---
    Search/list purchase returns
-   `API-PUR-013` GET
    `/api/v1/procurement/purchase-returns/{purchaseReturnId}` --- Get
    purchase return
-   `API-PUR-015` POST
    `/api/v1/procurement/purchase-returns/{purchaseReturnId}:post` ---
    Post purchase return atomically
...
-   `API-PUR-005`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT, PERMISSION_DENIED, RESOURCE_NOT_FOUND`.
-   `API-PUR-015`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT, PERMISSION_DENIED, RESOURCE_NOT_FOUND`.
