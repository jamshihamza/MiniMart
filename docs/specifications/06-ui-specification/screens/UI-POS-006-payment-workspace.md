...
Cash, manual card/QR and supported tender orchestration.
...
`payment.credit.use`, `payment.read`, `payment.write`

## Layout / information hierarchy

1.  Context/header: Store/Counter/Business Date when relevant, user,
    local/cloud state.
2.  Primary task region: Payment Workspace.
...
-   `API-PAY-007` POST
    `/api/v1/checkout-payments/{checkoutPaymentId}:cancel-uncommitted`
    --- Cancel safely uncommitted payment components
-   `API-PAY-008` GET
    `/api/v1/payment-commitments/{paymentCommitmentId}` --- Get durable
    confirmed pre-posting commitment
-   `API-PAY-009` GET `/api/v1/posted-tenders` --- Search immutable
    posted tenders
-   `API-PAY-014` POST
    `/api/v1/checkout-payments/{checkoutPaymentId}/credit-components`
    --- Add customer-credit component subject to authoritative credit
    evaluation
-   `API-PAY-016` GET
    `/api/v1/payment-commitments/{paymentCommitmentId}/recovery` --- Get
    commitment recovery/reversal status
...
-   `API-PAY-006`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT, PAYMENT_ALREADY_COMMITTED, PAYMENT_RECOVERY_REQUIRED`.
-   `API-PAY-007`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT, PAYMENT_ALREADY_COMMITTED, PAYMENT_RECOVERY_REQUIRED`.
-   `API-PAY-014`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT, PAYMENT_ALREADY_COMMITTED, PAYMENT_RECOVERY_REQUIRED`.
