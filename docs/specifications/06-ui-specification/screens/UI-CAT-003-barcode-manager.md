...
Maintain tenant/company-scoped barcodes.
...
1.  Context/header: Store/Counter/Business Date when relevant, user,
    local/cloud state.
2.  Primary task region: Barcode Manager.
...
-   `API-CAT-007` GET `/api/v1/catalog/barcodes/{barcode}:resolve` ---
    Resolve active barcode to sellable item snapshot
-   `API-CAT-008` POST `/api/v1/catalog/items/{itemId}/barcodes` --- Add
    barcode
-   `API-CAT-009` DELETE
    `/api/v1/catalog/items/{itemId}/barcodes/{barcodeId}` --- Remove
    barcode association where allowed
...
-   `API-CAT-006`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT, PERMISSION_DENIED, RESOURCE_NOT_FOUND`.
-   `API-CAT-008`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT, PERMISSION_DENIED, RESOURCE_NOT_FOUND`.
-   `API-CAT-009`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT, PERMISSION_DENIED, RESOURCE_NOT_FOUND`.
