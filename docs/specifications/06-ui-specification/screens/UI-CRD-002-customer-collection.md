# UI-CRD-002 --- Customer Collection

**Surface:** Back Office\
**Area:** Credit\
**Primary roles/personas:** Accounts, Cashier, Manager\
**Offline behavior:** READ_WRITE_LOCAL

## Purpose

Post collection and allocation.
...
`credit.collection.reverse`, `credit.read`, `credit.write`

## Layout / information hierarchy

1.  Context/header: Store/Counter/Business Date when relevant, user,
    local/cloud state.
2.  Primary task region: Customer Collection.
...
-   `API-CRD-007` GET `/api/v1/customer-collections/{collectionId}` ---
    Get customer collection
-   `API-CRD-009` POST
    `/api/v1/customer-collections/{collectionId}:reverse` --- Post
    compensating reversal for posted customer collection
...
-   `API-CRD-006`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, CREDIT_CHANGED, CREDIT_POLICY_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT`.
-   `API-CRD-009`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, CREDIT_CHANGED, CREDIT_POLICY_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT`.
