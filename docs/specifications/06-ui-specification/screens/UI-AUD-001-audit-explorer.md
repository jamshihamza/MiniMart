# UI-AUD-001 --- Audit Explorer

**Surface:** Back Office\
**Area:** Audit\
**Primary roles/personas:** Admin, Audit, Management\
...
permission/capability data. Relevant permission codes: `audit.read`

## Layout / information hierarchy

1.  Context/header: Store/Counter/Business Date when relevant, user,
    local/cloud state.
2.  Primary task region: Audit Explorer.
...
-   `API-AUD-001` GET `/api/v1/audit/events` --- Permission-controlled
    search of audit events
-   `API-AUD-002` GET `/api/v1/audit/events/{auditEventId}` --- Get
    audit event
