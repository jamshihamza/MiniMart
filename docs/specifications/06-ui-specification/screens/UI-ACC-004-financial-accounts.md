...
1.  Context/header: Store/Counter/Business Date when relevant, user,
    local/cloud state.
2.  Primary task region: Financial Accounts.
...
-   `API-ACC-007` GET `/api/v1/accounting/financial-accounts` ---
    List/search financial accounts
-   `API-ACC-008` POST `/api/v1/accounting/financial-accounts` ---
    Create financial account master
-   `API-ACC-009` PATCH
    `/api/v1/accounting/financial-accounts/{financialAccountId}` ---
    Update financial account master
...
-   `API-ACC-006`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT, PERMISSION_DENIED, RESOURCE_NOT_FOUND`.
-   `API-ACC-008`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT, PERMISSION_DENIED, RESOURCE_NOT_FOUND`.
-   `API-ACC-009`: disable duplicate submit; preserve idempotency
    identity; handle
