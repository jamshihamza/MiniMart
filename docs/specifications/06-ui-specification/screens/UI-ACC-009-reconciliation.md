...
Reconciliation session and matching workflow.
...
1.  Context/header: Store/Counter/Business Date when relevant, user,
    local/cloud state.
2.  Primary task region: Reconciliation.
...
-   `API-ACC-021` GET `/api/v1/accounting/reconciliations` --- Search
    reconciliation sessions
-   `API-ACC-022` POST `/api/v1/accounting/reconciliations` --- Create
    reconciliation session
-   `API-ACC-023` PATCH
    `/api/v1/accounting/reconciliations/{reconciliationId}` --- Update
    matches/differences while open
-   `API-ACC-024` POST
    `/api/v1/accounting/reconciliations/{reconciliationId}:complete` ---
    Complete reconciliation without rewriting source movements
...
-   `API-ACC-006`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT, PERMISSION_DENIED, RESOURCE_NOT_FOUND`.
-   `API-ACC-022`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT, PERMISSION_DENIED, RESOURCE_NOT_FOUND`.
-   `API-ACC-023`: disable duplicate submit; preserve idempotency
    identity; handle
...
-   `API-ACC-024`: disable duplicate submit; preserve idempotency
    identity; handle
    `AUTHENTICATION_REQUIRED, BUSINESS_RULE_REJECTED, IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST, INCOMPATIBLE_CLIENT, PERMISSION_DENIED, RESOURCE_NOT_FOUND`.
